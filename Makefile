...
app.name=Follower Count
jar.name=spark-hw1.jar
maven.jar.name=spark-hw1-1.0.jar
job.name=wc.WordCountMain
local.master=local[4]
local.input=/Users/visheshhemnani/Documents/CS_6240_LS_PDP/data/Twitter-dataset/data
# local.input=input
local.output=output
local.log=log
...
# Do not edit the bucket name, edit the project name instead
...
upload-app-aws:
	aws s3 cp ${jar.name} s3://${aws.bucket.name}/${aws.project.name}/${aws.project.version}/

# Main EMR launch.
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "TwitterFollower Spark Cluster Version ${aws.project.version}" \
		--release-label ${aws.emr.release} \
		--instance-groups '[{"InstanceCount":${aws.num.nodes},"InstanceGroupType":"CORE","InstanceType":"${aws.instance.type}"},{"InstanceCount":1,"InstanceGroupType":"MASTER","InstanceType":"${aws.instance.type}"}]' \
	    --applications Name=Hadoop Name=Spark \
		--steps Type=CUSTOM_JAR,Name="${app.name}",Jar="command-runner.jar",ActionOnFailure=TERMINATE_CLUSTER,Args=["spark-submit","--deploy-mode","cluster","--class","${job.name}","s3://${aws.bucket.name}/${aws.project.name}/${aws.project.version}/${jar.name}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.project.name}/${aws.project.version}/${aws.output}"] \
...
download-output-aws: clean-local-output
	mkdir -p ${local.output}/${aws.project.version}
